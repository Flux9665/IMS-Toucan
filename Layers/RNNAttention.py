...
class AttLoc2D(torch.nn.Module):
    ...
    def __init__(self, eprojs, dunits, att_dim, att_win, aconv_chans, aconv_filts, han_mode=False):
        ...
        self.mlp_dec = torch.nn.Linear(dunits, att_dim, bias=False)
        self.mlp_att = torch.nn.Linear(aconv_chans, att_dim, bias=False)
        self.loc_conv = torch.nn.Conv2d(1, aconv_chans, (att_win, 2 * aconv_filts + 1), padding=(0, aconv_filts), bias=False, )
        self.gvec = torch.nn.Linear(att_dim, 1)
...
class AttLocRec(torch.nn.Module):
    ...
    This attention is an extended version of location aware attention.
    With the use of RNN,
    it takes the effect of the history of attention weights into account.
...
class AttMultiHeadDot(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev):
        ...
        if self.pre_compute_k is None or self.han_mode:
            ...
            self.h_length = self.enc_h.size(1)
            # utt x frame x att_dim
            self.pre_compute_k = [torch.tanh(self.mlp_k[h](self.enc_h)) for h in range(self.aheads)]
        ...
        for h in range(self.aheads):
            e = torch.sum(self.pre_compute_k[h] * torch.tanh(self.mlp_q[h](dec_z)).view(batch, 1, self.att_dim_k), dim=2, )  # utt x frame
...
class AttMultiHeadAdd(torch.nn.Module):
    ...
    def forward(self, enc_hs_pad, enc_hs_len, dec_z, att_prev):
        ...
        w = []
        for h in range(self.aheads):
            e = self.gvec[h](torch.tanh(self.pre_compute_k[h] + self.mlp_q[h](dec_z).view(batch, 1, self.att_dim_k))).squeeze(2)
